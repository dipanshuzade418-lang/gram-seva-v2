...
## Two more bugs I found and fixed while reviewing the whole project

**1. `PatientDashboard.jsx` used a plain `<a href="/doctors">` instead of React Router's `<Link>`.**
That forced a full browser page reload instead of client-side navigation — which on GitHub
Pages runs straight into bug #2 below. Fixed to use `<Link>`.

**2. No fix for GitHub Pages + client-side routing.**
Your app uses `BrowserRouter`, which needs the server to serve `index.html` for every
path (`/register`, `/doctors`, `/doctors/123`, etc.). GitHub Pages can't do that — it
only serves real files, so refreshing on any route other than `/` hit GitHub's own 404 page.
I added the standard fix:
- `frontend/public/404.html` — redirects any unknown path back to `/` with the original
  path stashed in a query string.
- A small script added to `frontend/index.html` — reads that query string and restores
  the real URL via `history.replaceState`, so React Router picks up where it should.

No action needed from you beyond rebuilding (`npm run build`) and redeploying — both files
